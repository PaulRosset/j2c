...
const fs = require("fs");
const Convert = require("./lib/main");
...
program
  ...
  .option(
    "--csv [file.csv]",
    "The csv file that will be saved. (If not existing , will create it)"
  )
  .option(
    "-f, --first",
    "Specify the name of the object at the first position (default = obj)"
  )
  .parse(process.argv);
...
if (argc.rawArgs.length <= 2 || argc.rawArgs.length > 8) {
  ...
  fs.readFile(json, "utf8", (err, data) => {
    ...
    fs.writeFile(csv, conv.onceFilled(), err => {
      ...
      console.log(
        chalk.green(
          `Location of the file : ${chalk.cyan(process.cwd() + "/" + csv)}`
        )
      );
    });
  });
